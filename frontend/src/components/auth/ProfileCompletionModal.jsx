...
export default function ProfileCompletionModal() {
  const { user, profile, showProfileCompletion, markProfileComplete } = useAuth();
  ...
  const [step, setStep] = useState(1); // 1 = profile info, 2 = password (optional)
  ...
  const handleSaveProfile = async () => {
    ...
    // If user also set a password, update it
  ...
  const handleSkip = async () => {
    // Skip but mark as completed so popup doesn't show again
    await markProfileComplete({
      full_name: formData.full_name || profile?.full_name || '',
    });
  };

  // ── Styles — match your existing dark theme ────────────────
  const overlayStyle = {
  ...
  const cardStyle = {
    ...
    maxWidth: '440px',
    overflow: 'hidden',
    animation: 'profilePopIn 0.3s cubic-bezier(0.34,1.56,0.64,1)',
  };
          ...
          <div style={{ padding: '24px 24px 0' }}>
            ...
            {profile?.avatar_url && (
              <img
                ...
                alt="profile"
                style={{ width: '52px', height: '52px', borderRadius: '50%', marginBottom: '14px', border: '2px solid #374151' }}
              />
            )}
            ...
            <p style={{ color: '#9CA3AF', fontSize: '14px', lineHeight: '1.6', marginBottom: '20px' }}>
              Welcome to NewVacancy! You signed in with Google.
              Add a few details to set up your profile.
            </p>
            ...
            <div style={{ display: 'flex', gap: '6px', marginBottom: '20px' }}>
              {[1, 2].map(s => (
                <div key={s} style={{
                  ...
                  background: step >= s ? '#F97316' : '#2A2D3A',
          ...
          <div style={{ padding: '0 24px 20px' }}>
              ...
              <>
                ...
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <input
                    ...
                    onBlur={e => e.target.style.borderColor = errors.full_name ? '#EF4444' : '#374151'}
                  />
                  {errors.full_name && (
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  >
                    {loading ? 'Saving...' : 'Save & Skip password'}
                  </button>
                </div>
              </>
            )}
            ...
            {step === 2 && (
              <>
                {/* Set password info */}
                <div style={{
                  background: '#0C1F35', border: '1px solid #1E3A5F',
                  borderRadius: '8px', padding: '12px 14px', marginBottom: '16px',
                }}>
                  <p style={{ color: '#93C5FD', fontSize: '13px', lineHeight: '1.6', margin: 0 }}>
                    💡 Set a password so you can also log in with email + password.
                    <br />
                    <span style={{ color: '#6B7280', fontSize: '12px' }}>
                      You can skip this — Google login will always work.
                    </span>
                  </p>
                </div>
                ...
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <input
                    ...
                    onFocus={e => e.target.style.borderColor = '#F97316'}
                    onBlur={e => e.target.style.borderColor = errors.newPassword ? '#EF4444' : '#374151'}
                  />
                  {errors.newPassword && (
                    <p style={{ color: '#EF4444', fontSize: '12px', marginTop: '4px' }}>{errors.newPassword}</p>
                  )}
                </div>
                ...
                <div style={{ marginBottom: '20px' }}>
                  ...
                  <input
                    ...
                    onFocus={e => e.target.style.borderColor = '#F97316'}
                    onBlur={e => e.target.style.borderColor = errors.confirmPassword ? '#EF4444' : '#374151'}
                  />
                ...
                {errors.submit && (
                  <div style={{ background: '#FEE2E2', borderRadius: '8px', padding: '10px', color: '#991B1B', fontSize: '13px', marginBottom: '12px' }}>
                    ⚠️ {errors.submit}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  <button
                    onClick={() => { if (validateStep2()) handleSaveProfile(); }}
                    disabled={loading}
                    style={{
                      flex: 1, padding: '12px',
