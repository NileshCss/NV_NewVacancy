...
export default function ChaptersManager() {
  ...
  const handleOpenModal = (chap = null) => {
    ...
    if (chap) {
      setFormData({ name: chap.name, display_order: chap.display_order || 0 })
    } else {
      setFormData({ name: '', display_order: chapters.length })
    }
    setIsModalOpen(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingChapter) {
        ...
        await createChapter({ ...formData, subject_id: selectedSubjectId })
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-muted)]">Manage chapters for a specific subject.</p>
        </div>
        <div className="flex flex-wrap gap-3 items-center">
          ...
          <select className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-lg p-2 text-sm text-[var(--text-primary)]"
                  value={selectedSubjectId} onChange={e => setSelectedSubjectId(e.target.value)} disabled={!selectedExamId}>
            <option value="">Select Subject</option>
            {subjects.map(sub => <option key={sub.id} value={sub.id}>{sub.name}</option>)}
          </select>
          <button onClick={() => handleOpenModal()} disabled={!selectedSubjectId} className="btn-primary flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50 text-sm">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl overflow-hidden">
        {loading && selectedSubjectId ? (
          ...
          <div className="p-8 text-center text-[var(--text-muted)]">Please select an exam and a subject first.</div>
        ) : chapters.length === 0 ? (
          <div className="p-8 text-center text-[var(--text-muted)]">No chapters found for this subject.</div>
        ) : (
